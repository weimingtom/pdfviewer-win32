...
char * outputname_c = "output.swf";

int args_callback_option_combine(char*name,char*val) {
    if(!strcmp(name,"c"))
    {
	config.clip = 1;
	return 0;
...
static struct options_t options_combine[] = {
{"o", "output"},
{"t", "stack"},
{"T", "stack1"},
{"m", "merge"},
...
int args_callback_longoption_combine(char*name,char*val) {
    return args_long2shortoption(options_combine, name, val);
}

int args_callback_command_combine(char*name, char*val) {
...
void args_callback_usage_combine(char *name)
{
    printf("\n");
...
int mainCombine(int argn, char *argv[])
{
    int fi;
    SWF master;
    SWF slave;
    ...
    if (config.alloctest)
    {
	char*bitmap = malloc_safe(sizeof(char)*65536);
...
//	makestackmaster(&newswf);
    ...
    fi = open(outputname_c, O_BINARY|O_RDWR|O_TRUNC|O_CREAT, 0777);
